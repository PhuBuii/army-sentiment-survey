...
function SoldiersContent() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "pending" | "An tâm" | "Dao động" | "Nguy cơ"
  >("all");
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const payload = rows
        .filter((r) => r.full_name && r.unit)
        .map((r) => ({ full_name: r.full_name, unit: r.unit }));
      if (payload.length > 0) {
        const supabase = createClient();
      ...
      toast.error("Lỗi upload, thử lại.");
    } finally {
      setUploading(false);
    }
  };
  ...
  const toggleSelectAll = () => {
    setSelectedIds(
      selectedIds.length === paginatedSoldiers.length
        ? []
        : paginatedSoldiers.map((s) => s.id),
    );
  };
  ...
  const handleExportQRExcel = async () => {
    ...
    if (pendingList.length === 0) {
      toast.warn("Tất cả chiến sĩ đã nộp bài.");
      return;
    }
    const toastId: Id = toast.loading(
      `Đang tạo QR cho ${pendingList.length} chiến sĩ...`,
    );
    try {
      ...
      toast.update(toastId, {
        render: "Xuất file Excel thành công!",
        type: "success",
        isLoading: false,
        autoClose: 3000,
      });
            ...
            <Table className="min-w-[640px]">
              <TableHeader>
                <TableRow className="bg-slate-50/50 dark:bg-white/[0.02] border-slate-100 dark:border-white/8 hover:bg-transparent dark:hover:bg-transparent">
                  ...
                  <TableHead className="w-10 text-center py-3 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    #
                  </TableHead>
                  <TableHead className="py-3 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    Họ tên
                  </TableHead>
                  <TableHead className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    Đơn vị
                  </TableHead>
                  <TableHead className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    Trạng thái
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell
                      colSpan={7}
                      className="h-40 text-center text-sm text-slate-400 dark:text-slate-500"
                    >
                  ...
                  paginatedSoldiers.map((s, idx) => {
                    ...
                    return (
                      <TableRow
                        key={s.id}
                        className="hover:bg-slate-50/80 dark:hover:bg-white/[0.03] border-slate-100 dark:border-white/8"
                      >
                        ...
                        <TableCell className="font-medium text-slate-900 dark:text-slate-100 text-sm py-3 whitespace-nowrap">
                          {s.full_name}
                        </TableCell>
                        <TableCell
                          className="text-slate-500 dark:text-slate-400 text-xs max-w-[180px] truncate py-3"
                          title={s.unit}
                        >
                          {s.unit}
                        </TableCell>
                        <TableCell className="py-3">
                          {!s.is_completed ? (
                            <span className="text-xs text-slate-400 dark:text-slate-500 px-2 py-0.5 bg-slate-100 dark:bg-white/5 rounded-full w-max inline-block">
                              Chờ làm bài
                            </span>
                          ) : sub?.ai_status ? (
                            <StatusBadge status={sub.ai_status} />
                          ) : (
                            <span className="text-xs font-medium text-blue-600 dark:text-blue-400 px-2 py-0.5 bg-blue-50 dark:bg-blue-500/10 border border-blue-200/50 dark:border-blue-500/20 rounded-full flex items-center gap-1 w-max">
        ...
        <DialogContent className="max-w-md w-[96vw] rounded-2xl bg-white dark:bg-[#161b22] dark:border-white/10">
          <DialogHeader>
            <DialogTitle className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              {editTarget ? "Sửa thông tin" : "Thêm chiến sĩ"}
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-400">
              {editTarget
                ? `Đang sửa: ${editTarget.full_name}`
                : "Tạo hồ sơ chiến sĩ mới"}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 pt-2">
            ...
            <div className="flex gap-2 pt-1">
              <Button
                variant="outline"
                className="flex-1 h-10 rounded-xl dark:border-white/10"
                onClick={() => setEditDialogOpen(false)}
              >
                Huỷ
              </Button>
              <Button
                className="flex-1 h-10 rounded-xl bg-blue-600 hover:bg-blue-700 text-white gap-1.5"
            ...
            <div className="max-h-[72vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3 p-5 border-b border-slate-100 dark:border-white/8">
                <div className="bg-slate-50 dark:bg-white/[0.04] rounded-xl p-4 flex flex-col items-center border border-slate-100 dark:border-white/8">
                  <span className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-2">
                    Điểm tâm lý
                  </span>
                ...
                <div className="bg-slate-50 dark:bg-white/[0.04] rounded-xl p-4 flex flex-col items-center border border-slate-100 dark:border-white/8">
                  <span className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3">
                    Phân loại AI
                  </span>
                  {editingStatus ? (
                  ...
                  {editingStatus ? (
                    <>
                      <Button
                        ...
                        className="h-8 text-xs rounded-lg border-slate-200 dark:border-white/10"
                      >
                        Huỷ
                      </Button>
                      <Button
              ...
              <div className="px-5 py-4 space-y-4 border-b border-slate-100 dark:border-white/8">
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <Brain className="w-3.5 h-3.5 text-emerald-500" /> AI Nhận
                    xét
                  </h4>
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <ChevronDown className="w-3.5 h-3.5 text-amber-500" /> Lời
                    khuyên
                  </h4>
              ...
              <div className="px-5 py-4 border-b border-slate-100 dark:border-white/8">
                <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5 mb-2">
                  <FileText className="w-3.5 h-3.5 text-blue-500" /> Nhận xét
                  Chỉ huy
                </h4>
                {editingStatus ? (
                  <textarea
                  ...
                  <div className="text-sm text-slate-600 dark:text-slate-300 bg-white dark:bg-white/[0.03] rounded-xl p-3 border border-slate-100 dark:border-white/8 min-h-[60px]">
                    {sub.admin_note ? (
                      ...
                      <em className="text-slate-400 dark:text-slate-600">
                        Chưa có nhận xét.
                      </em>
                    )}
                  </div>
              ...
              <div className="px-5 py-4 space-y-3">
                <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <MessageSquare className="w-3.5 h-3.5 text-blue-500" /> Câu
                  trả lời ({sub.responses?.length ?? 0})
                </h4>
                {(sub.responses ?? []).map((r, i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-slate-100 dark:border-white/8 overflow-hidden"
                  >
                    ...
                    <div className="px-4 py-3">
                      <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold uppercase tracking-wider mb-1">
                        Trả lời
                      </p>
                      <p className="text-sm text-slate-800 dark:text-slate-100 leading-relaxed">
                        {r.answer || (
                          <em className="text-slate-400">Không có</em>
                        )}
                      </p>
                    </div>
                  </div>
    ...
    <React.Suspense
      fallback={
        <div className="p-10 text-center">
          <Loader2 className="animate-spin mx-auto mb-2" /> Đang tải danh
          sách...
        </div>
      }
    >
