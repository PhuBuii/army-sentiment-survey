...
import { markSubmissionResolved, getUnits } from "@/app/actions/admin-actions";
import {
  Users,
  Target,
  Activity,
...
function DashboardContent() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const { units: globalUnits } = await getUnits();
      const uniqueUnits = Array.from(
        new Set([
          ...globalUnits,
          ...soldiersData.map((s) => s.unit)
        ])
      ).filter(Boolean);
      setUnits(uniqueUnits);

      const filteredSoldiersData =
        selectedUnit === "all"
          ? soldiersData
          : soldiersData.filter((s) => s.unit === selectedUnit);
      const total = filteredSoldiersData.length;
      const completed = filteredSoldiersData.filter(
        (s) => s.is_completed,
      ).length;
      ...
      filteredSoldiersData
        .filter(
          (s) => s.is_completed && s.submissions && s.submissions.length > 0,
        )
        .forEach((s) => {
      ...
      const recent = [...filteredSoldiersData]
        ...
        .sort(
          (a, b) =>
            new Date(b.submissions![0].created_at).getTime() -
            new Date(a.submissions![0].created_at).getTime(),
        );
  ...
  const paginatedSoldiers = recentSoldiers.slice(
  ...
  const handleToggleResolved = async (checked: boolean) => {
    ...
    if (res.error) {
      ...
      toast.success(
        checked
          ? "Đã đánh dấu hoàn tất xử lý tư tưởng."
          : "Đã huỷ đánh dấu xử lý.",
      );
      const updatedSoldier = {
        ...selectedSoldier,
        submissions: [{ ...sub, is_resolved: checked }],
      };
      setSelectedSoldier(updatedSoldier);
      const newList = recentSoldiers.map((s) =>
      ...
      <div className="hidden print-only">
        <div className="flex justify-between items-start mb-10">
          <div className="text-center font-serif">
            ...
            <p className="text-[12px] font-bold underline underline-offset-4">
              LỮ ĐOÀN 77
            </p>
          </div>
          <div className="text-center font-serif flex flex-col items-center">
            ...
            <p className="text-[14px] italic">
              <span
                ...
                className="outline-none border-b border-transparent hover:border-gray-300 focus:border-gray-400"
              >
                {printLocation}
              </span>
        ...
        <div className="text-center mb-10">
          <h1 className="text-2xl font-bold uppercase mb-2 font-serif">
            BÁO CÁO TỔNG HỢP TÌNH HÌNH TÂM LÝ CHIẾN SĨ
          </h1>
        </div>

        <div className="mb-8">
          <h2 className="text-lg font-bold uppercase mb-4  bg-slate-50 font-serif">
            I. Thống kê chung
          </h2>
        ...
        <div className="mb-10">
          ...
          <table className="w-full border-collapse border border-black font-serif">
            ...
            <tbody>
              {recentSoldiers.filter(
                (s) => s.submissions?.[0]?.ai_status === "Nguy cơ",
              ).length > 0 ? (
                recentSoldiers
                  .filter((s) => s.submissions?.[0]?.ai_status === "Nguy cơ")
                  .map((s, i) => (
                    <tr key={s.id}>
                      ...
                      <td className="text-center font-bold">
                        {s.submissions?.[0]?.ai_score}
                      </td>
                      <td className="text-justify text-[11px]">
                        <p className="mb-1">
                          <b>[NHẬN XÉT]:</b>{" "}
                          <i>{s.submissions?.[0]?.ai_summary}</i>
                        </p>
                        <p>
                          <b>[XỬ LÝ]:</b> {s.submissions?.[0]?.ai_advice}
                        </p>
                      </td>
                    </tr>
                  ))
                ...
                <tr>
                  <td colSpan={5} className="text-center py-4 italic">
                    Không có dữ liệu nguy cơ.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="mb-10">
          <h2 className="text-lg font-bold uppercase mb-4  bg-slate-50 font-serif">
            III. Danh sách chi tiết chiến sĩ đã tham gia
          </h2>
          <table className="w-full border-collapse border border-black font-serif">
            <thead>
              <tr className="bg-slate-100">
            ...
            <tbody>
              {recentSoldiers.length > 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-4 italic">
                    Chưa có chiến sĩ nào hoàn thành khảo sát.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="mb-10">
          <h2 className="text-lg font-bold uppercase mb-4  bg-slate-50 font-serif">
            IV. Danh sách chiến sĩ chưa tham gia khảo sát
          </h2>
          <table className="w-full border-collapse border border-black font-serif">
            <thead>
              <tr className="bg-slate-100">
              ...
              {pendingSoldiers.length > 0 ? (
                pendingSoldiers.map((s, i) => (
                  <tr key={s.id}>
                    ...
                    <td className="font-bold">{s.full_name.toUpperCase()}</td>
                    <td className="text-center">{s.unit}</td>
                    <td className="text-center italic text-slate-500">
                      Chưa làm bài
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={4} className="text-center py-4 italic">
                    Toàn bộ chiến sĩ đã hoàn thành khảo sát.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-1 sm:px-0">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              Phân tích tư tưởng chiến sĩ thời gian thực.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            ...
            <CustomSelect
              ...
              options={[
                { label: "Toàn bộ Đơn vị", value: "all" },
        ...
        <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <Card className="shadow-sm border-slate-200 dark:border-white/8 bg-white dark:bg-[#161b22] hover:border-blue-500/30 transition-colors">
            <CardContent className="p-4 sm:p-5 flex items-center justify-between">
              <div>
                <p className="text-[10px] sm:text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">
                  Tổng quân số
                </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <Card className="lg:col-span-2 shadow-sm border-slate-200 dark:border-white/8 bg-white dark:bg-[#161b22] flex flex-col">
            <CardHeader className="pb-2 p-4 sm:p-6 flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1">
                  Điểm trung bình toàn đơn vị theo ngày
                </p>
              </div>
            ...
            <CardContent className="flex-1 p-4 sm:p-6 pt-0">
              {trendData.length > 0 ? (
                <div className="w-full h-55 sm:h-[250px] mt-4">
                ...
                <div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 py-10">
                  ...
                  <p className="text-xs">
                    Chưa có đủ dữ liệu để vẽ biểu đồ diễn biến.
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <Card className="lg:col-span-1 shadow-sm border-slate-200 dark:border-white/8 bg-white dark:bg-[#161b22] flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col items-center justify-center min-h-[280px] sm:min-h-[300px] p-4 sm:p-6 pt-0">
              {chartData.length > 0 ? (
                <div className="w-full h-55 sm:h-[250px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={chartData}
                ...
                <div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 py-10">
                  ...
                  <p className="text-xs text-center px-4">
                    Chưa có dữ liệu thống kê để hiển thị biểu đồ.
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <Card className="lg:col-span-3 shadow-sm border-slate-200 dark:border-white/8 bg-white dark:bg-[#161b22] overflow-hidden flex flex-col">
            <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 dark:border-white/8 p-4 sm:p-6 py-3.5 sm:py-4">
              ...
              <Link
                href="/admin/soldiers"
                className="text-[12px] sm:text-[13px] text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-medium flex items-center gap-1"
              >
                ...
                <Table className="min-w-[600px] sm:min-w-full">
                  <TableHeader>
                    <TableRow className="bg-slate-50/50 dark:bg-white/5 hover:bg-slate-50/50 dark:hover:bg-white/5 border-none">
                      <TableHead className="py-3 sm:py-4 w-12 text-center text-[11px] uppercase tracking-wider">
                        STT
                      </TableHead>
                      ...
                      <TableHead className="text-right text-[11px] uppercase tracking-wider pr-4 sm:pr-6">
                        Thao tác
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                        ...
                        <TableCell colSpan={5} className="h-48 text-center">
                          <div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 space-y-3">
                            ...
                            <p className="text-sm font-medium">
                              Chưa có bài khảo sát nào được hoàn thành.
                            </p>
                            <p className="text-xs opacity-70">
                              Dữ liệu sẽ xuất hiện tại đây khi có kết quả mới.
                            </p>
                          </div>
                        </TableCell>
                      </TableRow>
                      ...
                      paginatedSoldiers.map((soldier, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="font-semibold text-slate-900 dark:text-white text-xs sm:text-sm whitespace-nowrap">
                              {soldier.full_name}
                            </TableCell>
                            <TableCell
                              className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs max-w-[100px] sm:max-w-[200px] truncate"
                              title={soldier.unit}
                            >
                              {soldier.unit}
                            </TableCell>
          ...
          {selectedSoldier?.submissions?.[0] && (
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              <div className="grid grid-cols-2 gap-4 text-center">
                ...
                <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                  ...
                  <p className="text-3xl font-black">
                    {selectedSoldier.submissions[0].ai_score}
                  </p>
                </div>
              </div>
              ...
              {selectedSoldier.submissions[0].ai_status === "Nguy cơ" && (
                <div
                  className={`p-4 rounded-xl border flex gap-3 items-start ${selectedSoldier.submissions[0].is_resolved ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}
                >
                  <Checkbox
                    id="resolve-issue"
                    checked={!!selectedSoldier.submissions[0].is_resolved}
                    onCheckedChange={(checked) =>
                      handleToggleResolved(checked as boolean)
                    }
                    disabled={resolving}
                  />
                  <Label
                    htmlFor="resolve-issue"
                    className="cursor-pointer font-bold"
                  >
                    {selectedSoldier.submissions[0].is_resolved
                      ? "Đã hoàn tất can thiệp"
              ...
              <div>
                <h4 className="font-bold text-sm mb-2">AI Nhận Xét</h4>
                <p className="text-sm bg-emerald-50 dark:bg-emerald-500/10 p-4 rounded-xl">
                  {selectedSoldier.submissions[0].ai_summary}
                </p>
              </div>
              <div>
                <h4 className="font-bold text-sm mb-2">Lời Khuyên</h4>
                <p className="text-sm bg-amber-50 dark:bg-amber-500/10 p-4 rounded-xl">
                  {selectedSoldier.submissions[0].ai_advice}
                </p>
              </div>
              {selectedSoldier.submissions[0].ai_dialogue_script && (
                <div>
                  <h4 className="font-bold text-sm mb-2">Kịch Bản Đối Thoại</h4>
                  <div className="text-sm bg-blue-50 dark:bg-blue-500/10 p-4 rounded-xl whitespace-pre-line">
                    {selectedSoldier.submissions[0].ai_dialogue_script}
                  </div>
                </div>
              )}
    ...
    <Suspense
      fallback={
        <div className="p-10 text-center">
          <Loader2 className="animate-spin mx-auto mb-2" /> Đang khởi tạo
          Dashboard...
        </div>
      }
    >
