...
export async function POST(req: Request) {
  try {
    ...
    if (surveyRateLimiter) {
      ...
      if (!success) {
        return NextResponse.json(
          { error: "Quá tải hệ thống. Vui lòng chậm lại (Rate Limited)." },
          { status: 429 },
        );
      }
    }
    ...
    if (!question || !answer) {
      return NextResponse.json(
        { error: "Thiếu dữ liệu (question/answer)" },
        { status: 400 },
      );
    }
    ...
    const systemPrompt =
      'Bạn là trợ lý tâm lý chuyên nghiệp phục vụ trong Quân đội. Chiến sĩ vừa trả lời cho câu hỏi: "' +
      question +
      '".' +
      ...
      "2. Đây là câu hỏi phụ thứ " +
      (followUpCount + 1) +
      " cho chủ đề này.\n" +
      (isLastChance
        ? "3. Đây là CƠ HỘI CUỐI CÙNG để hỏi về chủ đề này. Nếu cần hỏi, hãy đưa ra câu hỏi sâu sắc và bao quát nhất.\n"
        : "") +
    ...
    const { object } = await generateObject({
      ...
      schema: z.object({
        needsFollowUp: z
          .boolean()
          .describe(
            "Trả về true nếu phát hiện tâm lý bất ổn cần khơi gợi thêm.",
          ),
        followUpQuestion: z
          .string()
          .describe(
            "Nếu needsFollowUp là true, sinh ra câu hỏi phụ ở đây. Nếu false, để chuỗi rỗng.",
          )
          .optional(),
      }),
    ...
    return NextResponse.json(
      { error: msg || "Lỗi khi xử lý hội thoại AI" },
      { status: 500 },
    );
  }
}
