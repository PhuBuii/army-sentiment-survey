...
export default function SurveyForm({
  ...
  const [lastCheckedAnswers, setLastCheckedAnswers] = useState<
      ...
      if (isFailed) {
        setErrorStr(
          submissionResult.error ||
            "Hệ thống đang bận hoặc có lỗi xảy ra. Vui lòng thử lại sau.",
        );
        setPhase("preview");
  ...
  const handleNext = async () => {
    ...
    while (
      currentSystemQIndex >= 0 &&
      localQuestions[currentSystemQIndex].isFollowUp
    ) {
      currentSystemQIndex--;
    }

    // previously we calculated follow-ups here; that value was unused
    ...
    try {
      ...
      const currentParentId = isCurrentFollowUp
        ? localQuestions[currentStep].parentId
        : localQuestions[currentStep].id;
      ...
      // Đếm số lượng câu hỏi phụ (follow-ups) đã được tạo cho câu hỏi chính (parent) này
      const currentTotalFollowUps = localQuestions.filter(
        (q) => q.parentId === currentParentId,
      ).length;

      // Giới hạn số lượng câu hỏi phụ tối đa là 2 câu
      if (currentTotalFollowUps < 2) {
        const res = await fetch("/api/interview", {
          ...
          body: JSON.stringify({
            ...
            answer: currentAns,
            followUpCount: currentTotalFollowUps, // Gửi số lượng hiện tại cho AI biết
          }),
        });
      ...
      console.error("AI Interview Error:", msg);
      ...
      if (msg.includes("503") || msg.includes("high demand")) {
        ...
        setApiError(
          "Trợ lý AI đang bận, hệ thống sẽ chuyển sang câu hỏi tiếp theo.",
        );
  ...
  const handleSubmit = async () => {
    ...
    const payload = localQuestions.map((q) => ({
      question: q.content,
      answer: answers[q.id] ?? "",
    }));
    const result = await submitSurveyAndAnalyze(soldier.id, payload);
    setSubmissionResult(
      result || { error: "Không nhận được phản hồi từ máy chủ." },
    );
  };
      ...
      <div className="rounded-2xl overflow-hidden border border-emerald-200 dark:border-[#a3e635]/30 bg-white dark:bg-[#0a0f08] shadow-2xl animate-in fade-in zoom-in-95 duration-500">
        <div className="flex flex-col items-center text-center px-8 py-16 gap-6">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-base max-w-sm leading-relaxed">
            ...
            đã hoàn thành khảo sát. Hệ thống AI đã phân tích và ghi nhận kết
            quả. Chúc đồng chí hoàn thành tốt nhiệm vụ!
          </p>

          {/* Stats */}
          <div className="flex gap-6 pt-2">
            <div className="text-center">
              <div className="text-2xl font-bold text-emerald-600 dark:text-[#a3e635]">
      ...
      <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-[#4d6639]/60 bg-white dark:bg-[#0a0f08] shadow-2xl animate-in fade-in duration-500">
        <div className="flex flex-col items-center text-center px-8 py-16 gap-8">
          ...
          <div className="space-y-3">
            <p className="text-slate-500 dark:text-slate-400 text-sm uppercase tracking-widest font-mono">
              AI đang xử lý
            </p>
  ...
  if (phase === "preview") {
    return (
      <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-[#4d6639]/60 bg-white dark:bg-[#0a0f08] shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-400">
        {/* Header */}
        <div
          className={`px-6 py-5 border-b border-slate-100 dark:border-white/10 ${isCompleted ? "bg-blue-50/50 dark:bg-blue-500/10" : "bg-slate-50 dark:bg-[#1a2315]/60"}`}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-9 h-9 rounded-xl border flex items-center justify-center ${isCompleted ? "bg-blue-100/50 dark:bg-blue-500/20 border-blue-200 dark:border-blue-500/40" : "bg-blue-50 dark:bg-blue-500/15 border-blue-100 dark:border-blue-500/30"}`}
              >
                {isCompleted ? (
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                  Đồng chí {soldier.full_name} &middot; {soldier.unit}
                </p>
              </div>
            </div>
            {isCompleted && (
        ...
        <div className="px-6 py-5 space-y-4 max-h-[55vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-white/10">
          {localQuestions.map((q, i) => {
            ...
            return (
              <div
                key={q.id}
                className={`group rounded-xl border p-4 transition-all duration-200 ${isCompleted ? "border-blue-100 dark:border-blue-500/10 bg-blue-50/20 dark:bg-blue-500/5" : "border-slate-100 dark:border-white/8 bg-slate-50/60 dark:bg-white/4 hover:border-emerald-200 dark:hover:border-[#a3e635]/30"}`}
              >
                <div className="flex gap-3">
                  <span
                    className={`shrink-0 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center mt-0.5 ${isCompleted ? "bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400" : "bg-emerald-100 dark:bg-[#a3e635]/15 text-emerald-700 dark:text-[#a3e635]"}`}
                  >
                    {i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p
                        className={`text-xs font-medium leading-normal ${isAI ? "text-amber-600/80 dark:text-amber-400/80" : "text-slate-400 dark:text-slate-500"}`}
                      >
                        {q.content}
                      </p>
                    </div>
                    <p
                      className={`text-sm font-medium leading-relaxed break-words ${isCompleted ? "text-slate-700 dark:text-slate-300" : "text-slate-800 dark:text-slate-200"}`}
                    >
                      {answers[q.id] || (
          ...
          {isCompleted ? (
            <div className="w-full flex flex-col items-center gap-2 py-1">
              <p className="text-xs text-slate-400 dark:text-slate-500 italic">
                Câu trả lời đã được ghi nhận vào hệ thống và không thể thay đổi
              </p>
  ...
  return (
    <div className="relative">
      ...
      {showOfflineAlert && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 z-[100] animate-in fade-in slide-in-from-top-4 duration-500`}
        >
          <div
            className={`flex items-center gap-3 px-5 py-3 rounded-2xl border shadow-2xl backdrop-blur-md ${!isOnline ? "bg-amber-50/90 dark:bg-amber-950/90 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400" : "bg-emerald-50/90 dark:bg-emerald-950/90 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400"}`}
          >
            {!isOnline ? (
              <>
                <div className="relative">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] opacity-80 font-medium">
                    Tiến độ vẫn đang được tự động lưu vào máy
                  </span>
                </div>
              </>
            ) : (
      ...
      <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-[#4d6639]/60 bg-white dark:bg-[#0a0f08] shadow-2xl">
        ...
        <div className="px-0">
          <ProgressBar
            ...
            color={isCompleted ? "blue" : "emerald"}
          />
        </div>
        ...
        <div className="px-4 sm:px-8 pt-5 pb-4 border-b border-slate-100 dark:border-white/8 bg-slate-50/60 dark:bg-[#0d1409]/60">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
              <div
                className={`w-8 h-8 sm:w-9 sm:h-9 shrink-0 rounded-xl border flex items-center justify-center ${isCompleted ? "bg-blue-50 dark:bg-blue-500/15 border-blue-100 dark:border-blue-500/30" : "bg-emerald-50 dark:bg-[#a3e635]/15 border-emerald-100 dark:border-[#a3e635]/30"}`}
              >
                <MessageSquare
                  className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isCompleted ? "text-blue-600 dark:text-blue-400" : "text-emerald-600 dark:text-[#a3e635]"}`}
                />
              </div>
              ...
              {localQuestions.map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    i === currentStep
                      ? isCompleted
                        ? "w-4 h-2 sm:w-5 sm:h-2.5 bg-blue-500"
                        : "w-4 h-2 sm:w-5 sm:h-2.5 bg-emerald-500 dark:bg-[#a3e635]"
                      : answers[localQuestions[i].id]?.trim()
                        ? isCompleted
                          ? "w-2 h-2 sm:w-2.5 sm:h-2.5 bg-blue-300 dark:bg-blue-500/50"
                          : "w-2 h-2 sm:w-2.5 sm:h-2.5 bg-emerald-300 dark:bg-[#a3e635]/50"
        ...
        >
          ...
          <div className="flex flex-col mb-5">
            {isMounted && currentStep === 0 && (
              <span className="text-emerald-600 dark:text-[#a3e635] text-[15px] font-bold mb-3 animate-in fade-in slide-in-from-left-4 duration-700">
                {(() => {
                  ...
                  if (hour >= 5 && hour < 11)
                    timeGreeting = "Chúc đồng chí buổi sáng huấn luyện tốt";
                  else if (hour >= 11 && hour < 14)
                    timeGreeting = "Chúc đồng chí nghỉ trưa thoải mái";
                  else if (hour >= 14 && hour < 18)
                    timeGreeting = "Chúc đồng chí buổi chiều năng lượng";
                  else if (hour >= 18 || hour < 5)
                    timeGreeting = "Chào đồng chí buổi tối";
            ...
            <div className="flex items-center justify-between">
              <p
                className={`text-xs font-bold uppercase tracking-widest flex items-center gap-2 font-mono ${isCompleted ? "text-blue-600" : "text-emerald-600 dark:text-[#a3e635]"}`}
              >
                <span
                  className={`inline-flex w-5 h-5 rounded-full items-center justify-center text-[10px] font-bold ${isCompleted ? "bg-blue-50 text-blue-600" : "bg-emerald-50 dark:bg-[#a3e635]/15 text-emerald-600 dark:text-[#a3e635]"}`}
                >
                  {currentStep + 1}
                </span>
          ...
          {/* Question text card */}
          <div
            className={`relative p-4 sm:p-5 rounded-2xl border transition-all duration-300 mb-5 sm:mb-6 ${isAI ? "bg-amber-50/30 dark:bg-amber-500/5 border-amber-100/50 dark:border-amber-500/20 shadow-sm" : "bg-slate-50/50 dark:bg-white/[0.02] border-slate-100 dark:border-white/5 shadow-sm"}`}
          >
            <div className="absolute top-2 right-3 opacity-10 dark:opacity-5 hidden sm:block">
              <MessageSquare size={64} />
            </div>
            <h3
              className={`relative z-10 text-lg sm:text-2xl font-bold leading-relaxed ${isAI ? "text-amber-700 dark:text-amber-400" : "text-slate-800 dark:text-white"}`}
            >
              {currentQ.content}
            </h3>
          </div>
          ...
          <div className="relative flex-1">
            <textarea
              ref={textareaRef}
              readOnly={isCompleted || isChecking}
              rows={4}
              ...
              placeholder={
                isCompleted
                  ...
                  : isChecking
                    ...
                    : "Công việc và tâm tư của đồng chí hôm nay như thế nào?..."
              }
              value={currentAns}
              onChange={(e) =>
                !isCompleted &&
          ...
          {!isCompleted && (
            <p className="text-xs  text-slate-300 dark:text-slate-600 mt-3 font-mono flex items-center justify-between">
              <span className="hidden sm:flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 rounded border border-slate-200 dark:border-white/10 text-[10px]">
                  Ctrl
                </kbd>{" "}
                +{" "}
                <kbd className="px-1.5 py-0.5 rounded border border-slate-200 dark:border-white/10 text-[10px]">
              ...
              {isChecking && (
                <span className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400 font-medium px-2.5 py-1 rounded ">
        ...
        <div className="flex items-center justify-between px-4 sm:px-8 py-4 sm:py-5 border-t border-slate-100 dark:border-white/8 bg-slate-50/40 dark:bg-[#0a0f08]/60 gap-3">
          ...
          <div className="flex items-center gap-2 flex-1 sm:flex-none justify-end">
            ...
            <span className="hidden lg:flex text-xs text-slate-400 dark:text-slate-500 items-center gap-1.5 mr-2">
              <CheckCircle2
                className={`w-3.5 h-3.5 ${isCompleted ? "text-blue-500" : "text-emerald-500 dark:text-[#a3e635]"}`}
              />
              {isCompleted
                ? "Đã xem hết"
                ...
                {isChecking ? (
                  ...
                  <>
                    ...
                    {isCompleted ? (
                      ...
                      <>
                        <span className="hidden sm:inline">Xem lại & </span>Nộp
                        bài
                      </>
                    )}
                  </>
              ...
              >
                {isChecking ? (
                  ...
                  <>
                    ...
                    <span className="inline">Tiếp theo</span>{" "}
